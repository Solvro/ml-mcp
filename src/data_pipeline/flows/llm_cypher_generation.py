...
from src.data_pipeline.deterministic_topology import stabilize_category_item_edges
...
def _stabilize_category_item_edges(parts: List[str], extracted_text: str, logger) -> List[str]:
    """Give every category -> item edge the page's type and one direction (#104).
    ...
    Returns:
        The statements with their category -> item edges rewritten
    """
    stabilized, rewrites = stabilize_category_item_edges(extracted_text, parts)

    if rewrites:
        logger.info(
            "Rewrote %d category -> item edge(s): %s", len(rewrites), "; ".join(rewrites[:10])
        )
...
def generate_cypher_queries(extracted_text: str, schema_context: str = "") -> str:
    ...
    parts = _stabilize_category_item_edges(parts, extracted_text, logger)
